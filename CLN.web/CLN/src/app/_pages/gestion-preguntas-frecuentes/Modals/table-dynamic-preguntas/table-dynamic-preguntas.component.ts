...
@Component({
  selector: 'app-table-dynamic-preguntas',
  templateUrl: './table-dynamic-preguntas.component.html',
  styleUrls: [
    "../../../../_shared/styles/tables.scss",
    './table-dynamic-preguntas.component.scss'
  ]
})
export class TableDynamicPreguntasComponent implements OnInit, OnChanges {
  ...
  public dataConcat(value: any, index) {
    if (value != undefined) {
      if (value.toLowerCase().toLowerCase().indexOf(" + ") != -1) {
        ...
        return (this.pageView[index][`${value}`]) ? (Object.prototype.toString.call(this.pageView[index][`${value}`]) == "[object String]") ? this.pageView[index][`${value}`] : (Object.prototype.toString.call(this.pageView[index][`${value}`]) == "[object Number]") ? this.pageView[index][`${value}`] : "" : "";
      }
    }
  }
  ...
  public activeAction(item) {
    return (item.estado == "Activo") ? "Inactivar" : (isNaN(item.estado)) ? "Activo" : (item.estado == 1) ? "Inactivar" : "Activo";
  }
