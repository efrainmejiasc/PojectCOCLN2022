...
@Component({
  selector: 'app-procesos-compra',
  templateUrl: './procesos-compra.component.html',
  styleUrls: ['./procesos-compra.component.scss']
})
export class ProcesosCompraComponent implements OnInit {

  // no se puede inlcuir el tipado porque rompo otras páginas de otros desarrolladores
  companyInterest:any = {
    idCompanyInterest: 0,
    idCompanyProfile: 0,
    interestType: "Procesos de compra publica",
  ...
  notificationCambios = "¿Desea salir sin guardar los cambios?"
  showAlertOut:boolean = false;
  salirSinGuardar:boolean;
  ...
  show( tipo: string ){
    ...
    if( tipo === 'procesos' ){
      if( !this.wrapperActiveProcesos ){
        this.wrapperActiveProcesos = true;
      }else{
        this.wrapperActiveProcesos = false;
      }
  ...
  getCompanyInterestData(){
    ...
    this.dataService.getCompanyInterest().subscribe((response:any)=>{
      ...
      response.data.forEach(company=>{
        if(company.interestType === "Procesos de compra publica"){
      ...
      if(this.companyInterest.classifiers){
        for(let i=0; i<this.companyInterest.classifiers.length; i++){
          ...
          let producto = {
            ...
            value:item.classifierCode + " - " +item.classifierName,
            selected:true
          };
  ...
  stringEntidad(word:string){
    this.layoutService.showLoading();
    this.entidadesFoundList = [];
    this.dataService.searchEntidad(word).subscribe( (response:any) =>{
      this.layoutService.closeLoading();
  ...
  checkRequiredFields(){
    ...
    if(this.stagesSelected){
      if(this.stagesSelected.length <1){
        this.guardarOn = false;
        this.showAlertStage = true;
      };
    };
  ...
  goAndSave(){
    ...
    this.companyInterest.interestType = "Procesos de compra publica";
...
function InterestObj(){
  ...
  this.updateMunicipalitiesList = ()=>{
    ...
    this.municipalitiesSelected = [];
    this.municipalitiesNoSelected = [];
    if(this.allMunicipalitiesSelected && this.departamentosSelected.length>0){
      this.idsMunicipalitiesSelected = [];
    }
